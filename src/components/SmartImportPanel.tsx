  ...
  return <section className="rounded-[2rem] border border-white/70 bg-white p-5 shadow-[0_24px_80px_rgba(0,70,34,0.2)] sm:p-7" aria-labelledby="smart-import-title">
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <div className="flex flex-col gap-3">
        ...
        <button onClick={copyRulesPrompt} className="min-h-11 rounded-xl border border-[#018f41] bg-emerald-50 px-4 text-sm font-extrabold text-[#01823c]">{copied ? "✓ Đã copy prompt" : "1. Copy prompt cho Gemini / AI ngoài"}</button>
        <label className="text-sm font-extrabold text-slate-700">2. Dán JSON bộ đề hoặc JSON quy tắc do Gemini trả về<textarea value={rulesRaw} onChange={(e) => { setRulesRaw(e.target.value); setPreview(null); }} placeholder='{ "title": "Tên bộ đề", "questions": [...] }' className="mt-2 min-h-28 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 p-3 font-mono text-xs font-normal text-slate-900 outline-none focus:border-[#018f41] focus:bg-white"/></label>
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-black uppercase tracking-wider text-amber-800">Đề và kết quả mẫu</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><a href="/samples/de-mau-gemini.txt" download className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-white px-3 text-xs font-extrabold text-amber-900 shadow-sm"><Download size={15}/>Tải đề mẫu</a><a href="/samples/ket-qua-gemini-mau.json" download className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-white px-3 text-xs font-extrabold text-amber-900 shadow-sm"><Download size={15}/>Tải JSON mẫu</a></div><details className="mt-2 rounded-xl bg-white p-2"><summary className="cursor-pointer px-2 py-1 text-xs font-extrabold text-[#01823c]">▶ Xem video hướng dẫn Gemini từ A–Z (1 phút)</summary><video className="mt-2 aspect-video w-full rounded-lg bg-slate-950" controls preload="metadata" poster="/tutorial/gemini-tutorial-poster.png"><source src="/tutorial/huong-dan-nhap-de-bang-gemini.mp4" type="video/mp4"/>Trình duyệt chưa hỗ trợ phát video MP4.</video><a href="/tutorial/huong-dan-nhap-de-bang-gemini.mp4" download className="mt-2 inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg bg-emerald-50 px-3 text-xs font-extrabold text-[#01823c]"><Download size={14}/>Tải video về máy</a></details></div>
