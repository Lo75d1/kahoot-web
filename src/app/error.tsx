  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-[#071b16] p-5 text-center text-white">
      <div className="max-w-md rounded-[2rem] bg-[#f3efdf] p-8 text-slate-800 shadow-2xl">
        ...
        <h1 className="mt-4 text-2xl font-extrabold text-[#01823c]">UDA Assessment Hub vừa gặp sự cố</h1>
        <p className="mt-2 text-sm leading-6 text-slate-600">Dữ liệu của bạn vẫn được giữ nguyên. Hãy thử tải lại phần này.</p>
        <button onClick={reset} className="mx-auto mt-5 inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#018f41] px-5 font-bold text-white hover:bg-[#01823c]"><RefreshCw size={18} />Thử lại</button>
      </div>
    </main>
  );
}
